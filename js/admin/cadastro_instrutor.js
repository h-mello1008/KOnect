document
  .getElementById("formCadastroInstrutor")
  .addEventListener("submit", async (e) => {
    ...
    const instrutorData = {
      ...
      horario_fechamento: form.querySelector('[name="horario_fechamento"]')
        .value,
      periodo_contrato: form.querySelector('[name="periodo_contrato"]:checked')
        .value,
      renovacao_automatica:
        form.querySelector('[name="renovacao_automatica"]').value === "sim"
          ? 1
          : 0,
    ...
    try {
      const response = await fetch(
        "../../../php/instrutor/instrutor_novo.php",
        {
          method: "POST",
          body: formData,
        },
