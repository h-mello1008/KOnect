...
function renderWithFilters() {
  ...
  if (status !== "all") {
    itens = itens.filter(
      (i) => i.status_pagamento === status || i.status_display === status,
    );
  }
...
function renderTabela(itens) {
  ...
  if (itens.length === 0) {
    body.innerHTML =
      '<tr><td colspan="8" class="text-center text-muted">Nenhuma mensalidade encontrada</td></tr>';
    return;
  }
  ...
  const rows = itens
    .map((m) => {
      ...
      const dias =
        m.dias_vencimento > 0
          ? `${m.dias_vencimento} dia${m.dias_vencimento > 1 ? "s" : ""}`
          : m.dias_vencimento === 0
            ? "Vence hoje"
            : `${Math.abs(m.dias_vencimento)} dias`;
