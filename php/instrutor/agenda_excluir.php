...
if ($id <= 0) {
    echo json_encode(['status' => 'nok', 'mensagem' => 'ID da aula não informado', 'data' => []]);
    exit;
}
...
$stmt = $conexao->prepare("UPDATE Agenda SET ativo = 0 WHERE id = ? AND academia_id = ? AND ativo = 1");
$stmt->bind_param("ii", $id, $academia_id);
$stmt->execute();

if ($stmt->errno === 0 && $stmt->affected_rows > 0) {
    echo json_encode(['status' => 'ok', 'mensagem' => 'Aula excluída com sucesso', 'data' => []]);
} else {
